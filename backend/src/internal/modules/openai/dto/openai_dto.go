...
func (m *ChatCompletionMessage) UnmarshalJSON(data []byte) error {
	...
	for _, p := range parts {
		...
		case "image_url", "input_image":
			...
			if attachment, ok := attachmentFromDataURL(p.ImageURL.URL, len(attachments)+1, p.Filename); ok {
...
func attachmentFromDataURL(value string, index int, filename string) (models.Attachment, bool) {
	...
	// 使用前端传来的原始文件名，保留正确的扩展名
	name := strings.TrimSpace(filename)
	if name == "" {
		name = fmt.Sprintf("file_%d%s", index, extensionFromMimeType(mimeType))
	}
...
type ImageGenerationRequest struct {
	...
	// Gemini conversation metadata (for image generation within an existing conversation)
	ConversationID string `json:"conversation_id,omitempty"`
	ResponseID     string `json:"response_id,omitempty"`
	ChoiceID       string `json:"choice_id,omitempty"`
}
