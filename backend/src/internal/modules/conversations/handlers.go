...
func (h *Handler) createConversation(c fiber.Ctx) error {
	...
	result, err := h.db.Exec(
		...
		userID, body.Title, body.Model, now, now,
	)
	if err != nil {
...
func (h *Handler) listConversations(c fiber.Ctx) error {
	...
	rows, err := h.db.Query(
		"SELECT id, user_id, title, model, created_at, updated_at FROM conversations WHERE user_id = ? ORDER BY updated_at DESC LIMIT ? OFFSET ?",
		userID, limit, offset,
	)
	if err != nil {
